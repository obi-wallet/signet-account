...
_build:
	RUSTFLAGS='-C link-arg=-s' cargo build --release --target wasm32-unknown-unknown --features "$(FEATURES)"
...
.PHONY: compress-wasm
compress-wasm:
	cp -f ./target/wasm32-unknown-unknown/release/*.wasm ./artifacts/
...
clean:
	cargo clean
	-rm -rf ./optimized-wasm/*.wasm.gz
	-rm -rf ./artifacts/*.wasm
	-rm -f ./contracts/secret-share-signer-local/tests/code_details.json
	-cp -f ./contracts/secret-share-signer-local/tests/code_details.empty.json ./contracts/secret-share-signer-local/tests/code_details.json