...
macros::cosmwasm_imports!(coins, to_binary, Binary, Coin, Empty, StdResult,);

#[cfg(feature = "stargate")]
cosmwasm_imports!(Decimal, IbcMsg,);
...
/// We need to implement Into when not on secret since our legacy messages
/// won't get correctly attached when executing unless converted
#[cfg(not(feature = "secretwasm"))]
impl<T> Into<cosmwasm_std::CosmosMsg<T>> for CosmosMsg {
    fn into(self) -> cosmwasm_std::CosmosMsg<T> {
        match self {
            CosmosMsg::Bank(msg) => cosmwasm_std::CosmosMsg::Bank(msg.into()),
            CosmosMsg::Custom(msg) => {
                panic!("Custom message conversion not yet supported");
            }
            #[cfg(feature = "staking")]
            ...
            CosmosMsg::Wasm(msg) => cosmwasm_std::CosmosMsg::Wasm(msg.into()),
            #[cfg(feature = "stargate")]
            CosmosMsg::Gov(msg) => cosmwasm_std::CosmosMsg::Gov(msg),
        }
    }
}
...
#[cfg(not(feature = "secretwasm"))]
impl Into<cosmwasm_std::BankMsg> for BankMsg {
    fn into(self) -> cosmwasm_std::BankMsg {
        match self {
            BankMsg::Send { to_address, amount } => {
                cosmwasm_std::BankMsg::Send { to_address, amount }
            }
            BankMsg::Burn { amount } => cosmwasm_std::BankMsg::Burn { amount },
        }
    }
}
...
#[cfg(not(feature = "secretwasm"))]
impl Into<cosmwasm_std::WasmMsg> for WasmMsg {
    fn into(self) -> cosmwasm_std::WasmMsg {
        match self {
            WasmMsg::Execute {
                contract_addr,
                msg,
...
mod tests {
    use super::*;
    macros::cosmwasm_imports!(coin, coins);
